...
double Media(vector<double> v, int inizio, int fine) {	
  double somma=0;
  for(unsigned int i=inizio; i<fine; i++) {
    somma+=v.at(i);
  }
  
  return somma/double(fine-inizio);
...
double Varianza(vector<double> v, int inizio, int fine) {
  double somma=0;	
  for(unsigned int i=inizio; i<fine; i++){
    somma+=pow(Media(v, inizio, fine)-v.at(i), 2.);
  }
