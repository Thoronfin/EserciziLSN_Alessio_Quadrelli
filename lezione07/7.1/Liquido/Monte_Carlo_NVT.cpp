...
{ 
  ...
  int nconf = 1;	
  Equilibrate(2000); //equilibrio sistema
  for(int iblk=1; iblk <= nblk; ++iblk) //Simulation
  {
	...
	cout << "Acceptance probability: " << accepted/attempted << endl;
  }
...
void Print_Istant(){
    ofstream Epot, Pres;
    Epot.open("istant.epot.0",ios::app);
    Pres.open("istant.pres.0",ios::app);
    //double potential = walker[iv]/(double)npart + vtail;
    double potential = walker[iv]/(double)npart;
    //double pres =  rho * temp + (walker[iw]+ ptail * (double)npart) / vol;
     double pres =  rho * temp + (walker[iw] / vol);
...
//g(r)  
    ...
    Gofr<<endl; //richiedo andare a capo a fine riga dei vari blocchi 
    cout << "----------------------------" << endl << endl;
