...
//	return k_elastic*val*val/2.0;
   return pow(val,4) - 2.5 * pow(val,2);     // Funzione d'onda uniforme
...
//	return 1.0;
  ...
  //Funzione d'onda con parametri ottimizati
      double _mi = 0.8;
      double _sigma = 0.6;
      double exp_dx = exp(-pow(v - _mi,2)/(2*_sigma*_sigma));
    	double exp_sx = exp(-pow(v + _mi,2)/(2*_sigma*_sigma));
...
//	return 0;
  ...
  double fac_1 = 1/(pow(_sigma,2));
	double fac_2 = 1/pow(_sigma,4);
	double arg_1 = pow(v - _mi ,2);
	double arg_2 = pow(v + _mi ,2);

  return (-exp_dx*fac_1 - exp_sx*fac_1 + exp_dx*fac_2*arg_1 + exp_sx*fac_2*arg_2);
...
{
	ofstream out("../probability_pigs_1T.dat");
